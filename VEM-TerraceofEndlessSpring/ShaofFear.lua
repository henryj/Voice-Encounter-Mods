...
local function spectimestart()
	if huddle == 1 and spout == 1 and strike == 0 then
		sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ylxjzb.ogg")
	...
	if huddle == 1 and spout == 0 and strike == 0 then
		sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\specialsoon.ogg")
		timerSpoStrCD:Start()
	end
	if huddle == 0 and spout == 1 and strike == 0 then
		timerHuddleInTerrorCD:Start()
		specWarningpreHud:Schedule(8)
		sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wszb.ogg")
		if not mod:IsTank() then
			sndWOP:Schedule(7, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.ogg")
		end
	end
			...
			if shaPower < 23.3 then
				mod:Schedule(23.3 - shaPower, function()
					if not warnedBreath then
						warnedBreath = true
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_tenkj.ogg") --10秒後恐懼之息
						VEM.Flash:Shake(1, 0, 0)
						sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.ogg")
						sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
						sndWOP:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
						sndWOP:Schedule(8.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
						sndWOP:Schedule(9.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
					end
				end)
			end
		end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(129147) then
		...
		if args:IsPlayer() then
			...
			if self:AntiSpam(2, 4) then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\telesoon.ogg")--準備傳送
			end
	...
	elseif args:IsSpellID(132007) then
		if not mod:IsDps() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_llj.ogg")--六連擊
		end
		ThrashCount = 0
		swingcount = -1
		timerThrashCD:Start()
	...
	elseif args:IsSpellID(120047) and MobID and MobID == args:GetSrcCreatureID() then--might change
		...
		if not self.Options.pscount then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kbpszb.ogg")--恐怖噴散
		end
	...
	elseif args:IsSpellID(131996) and not onPlatform then
		...
		specWarnThrash:Show()
		if not mod:IsDps() then
			sndDD:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\doubleat.ogg")--雙重攻擊	
			timerThrashCD:Start()
			if phase == 2 then
				...
				if inRange then
					self:Schedule(0.3, function()
						if self:AntiSpam(5, 20) then
							...
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\keepmove.ogg")
						end
					end)
 				end
 			end
	...
	elseif args:IsSpellID(120268) then
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyg.ogg") --轉移光
		end
		lastyongshi = args.destName
	elseif args:IsSpellID(120669) then--赤裸
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.ogg")
		...
		if mod:IsTank() then
			if timeryinmo:GetTime() < 20 then
				sndWOP:Schedule(26, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.ogg") --赤裸準備
	...
	elseif args:IsSpellID(120629) then
		...
		if self:AntiSpam(5, 6) then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wsks.ogg")
			specWarnweisuo:Show(kongjuCount)
			timerweisuo:Start(50, kongjuCount + 1)
			huddle = 1
			spectimestart()
			if kongjuCount % 4 == 1 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
			elseif kongjuCount % 4 == 2 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
			elseif kongjuCount % 4 == 3 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
			elseif kongjuCount % 4 == 0 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
			end
			if MyJS() then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.ogg") --注意減傷
			end
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(119888) and MobID and MobID == args:GetSrcCreatureID() then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jykd.ogg") --劍雨快躲
	elseif args:IsSpellID(120519) then
		specWarnshuipo:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_spzb.ogg") --水魄準備
	elseif args:IsSpellID(120455) then --隐没
		...
		sndWOP:Schedule(9, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\specialsoon.ogg") --準備特別技能
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ymzb.ogg")
	elseif args:IsSpellID(120458) then --浮現
		if yinmoCount == 1 then
			timerNakedAndAfraidCD:Start(14)
			if mod:IsTank() then
				sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.ogg") --赤裸準備
				sndWOP:Schedule(11.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
				sndWOP:Schedule(12.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
				sndWOP:Schedule(13.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
			end
		else
			if (timerNakedAndAfraidCD:GetTime() > 15) or (timerNakedAndAfraidCD:GetTime() == 0) then
				timerNakedAndAfraidCD:Update(15, 30)
				if mod:IsTank() then
					sndWOP:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.ogg") --赤裸準備
					sndWOP:Schedule(12.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
					sndWOP:Schedule(13.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
					sndWOP:Schedule(14.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
				end
			end
		end
	...
	elseif args:IsSpellID(120394) and self:AntiSpam(5, 1) then
		if UnitBuff("player", GetSpellInfo(120268)) then
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stilldanger.ogg")
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(119983) then
		...
		if self.Options.pscount then
			if kbpscount == 1 then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg") --恐怖噴散計數
		...
		if MobID == 61046 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 6) then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\moveleft.ogg")
				end
			end
		end	
		if MobID == 61042 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 7) then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\moveright.ogg")
				end
			end
		end	
		if MobID == 61038 then
		...
		if kbpscount == 16 then
			if VEM.ShaAssistStarModeChosed == "Dps" or VEM.ShaAssistStarModeChosed == "Healther" then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\end.ogg")
				end
			end
		end
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 119108 and self:AntiSpam(2, 3) then
		if not onPlatform then
			...
			if mod:IsDps() then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kdkjzz.ogg") --快打恐懼之子
			end
		end
	elseif spellId == 114936 and self:AntiSpam(5, 6) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_tenkj.ogg")
