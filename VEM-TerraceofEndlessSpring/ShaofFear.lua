...
local function spectimestart()
	if huddle == 1 and spout == 1 and strike == 0 then
		sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ylxjzb.mp3")
	...
	if huddle == 1 and spout == 0 and strike == 0 then
		sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\specialsoon.mp3")
		timerSpoStrCD:Start()
	end
	if huddle == 0 and spout == 1 and strike == 0 then
		timerHuddleInTerrorCD:Start()
		specWarningpreHud:Schedule(8)
		sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wszb.mp3")
		if not mod:IsTank() then
			sndWOP:Schedule(7, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.mp3")
		end
	end
			...
			if shaPower < 23.3 then
				mod:Schedule(23.3 - shaPower, function()
					if not warnedBreath then
						warnedBreath = true
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_tenkj.mp3") --10秒後恐懼之息
						VEM.Flash:Shake(1, 0, 0)
						sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
						sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
						sndWOP:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
						sndWOP:Schedule(8.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
						sndWOP:Schedule(9.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
					end
				end)
			end
		end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(129147) then
		...
		if args:IsPlayer() then
			...
			if self:AntiSpam(2, 4) then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\telesoon.mp3")--準備傳送
			end
	...
	elseif args:IsSpellID(132007) then
		if not mod:IsDps() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_llj.mp3")--六連擊
		end
		ThrashCount = 0
		swingcount = -1
		timerThrashCD:Start()
	...
	elseif args:IsSpellID(120047) and MobID and MobID == args:GetSrcCreatureID() then--might change
		...
		if not self.Options.pscount then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kbpszb.mp3")--恐怖噴散
		end
	...
	elseif args:IsSpellID(131996) and not onPlatform then
		...
		specWarnThrash:Show()
		if not mod:IsDps() then
			sndDD:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\doubleat.mp3")--雙重攻擊	
			timerThrashCD:Start()
			if phase == 2 then
	...
	elseif args:IsSpellID(120519) then --水魄
		...
		if args:IsPlayer() and self:AntiSpam(5, 20) then
			...
			if not UnitBuff("player", GetSpellInfo(120268)) then
				VEM.Flash:Shake(1, 0, 0)
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3")
			end
		else
				...
				if inRange then
					self:Schedule(0.3, function()
						if self:AntiSpam(5, 20) then
							...
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\keepmove.mp3")
						end
					end)
 				end
 			end
	...
	elseif args:IsSpellID(120268) then
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyg.mp3") --轉移光
		end
		lastyongshi = args.destName
	elseif args:IsSpellID(120669) then--赤裸
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3")
		...
		if mod:IsTank() then
			if timeryinmo:GetTime() < 20 then
				sndWOP:Schedule(26, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
	...
	elseif args:IsSpellID(120629) then
		...
		if self:AntiSpam(5, 6) then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wsks.mp3")
			specWarnweisuo:Show(kongjuCount)
			timerweisuo:Start(50, kongjuCount + 1)
			huddle = 1
			spectimestart()
			if kongjuCount % 4 == 1 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			elseif kongjuCount % 4 == 2 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			elseif kongjuCount % 4 == 3 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			elseif kongjuCount % 4 == 0 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			end
			if MyJS() then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
			end
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(119888) and MobID and MobID == args:GetSrcCreatureID() then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jykd.mp3") --劍雨快躲
	elseif args:IsSpellID(120519) then
		specWarnshuipo:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_spzb.mp3") --水魄準備
	elseif args:IsSpellID(120455) then --隐没
		...
		sndWOP:Schedule(9, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\specialsoon.mp3") --準備特別技能
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ymzb.mp3")
	elseif args:IsSpellID(120458) then --浮現
		if yinmoCount == 1 then
			timerNakedAndAfraidCD:Start(14)
			if mod:IsTank() then
				sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
				sndWOP:Schedule(11.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndWOP:Schedule(12.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndWOP:Schedule(13.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			end
		else
			if (timerNakedAndAfraidCD:GetTime() > 15) or (timerNakedAndAfraidCD:GetTime() == 0) then
				timerNakedAndAfraidCD:Update(15, 30)
				if mod:IsTank() then
					sndWOP:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
					sndWOP:Schedule(12.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
					sndWOP:Schedule(13.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
					sndWOP:Schedule(14.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
				end
			end
		end
	...
	elseif args:IsSpellID(120394) and self:AntiSpam(5, 1) then
		if UnitBuff("player", GetSpellInfo(120268)) then
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stilldanger.mp3")
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(119983) then
		...
		if self.Options.pscount then
			if kbpscount == 1 then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3") --恐怖噴散計數
		...
		if MobID == 61046 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 6) then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\moveleft.mp3")
				end
			end
		end	
		if MobID == 61042 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 7) then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\moveright.mp3")
				end
			end
		end	
		if MobID == 61038 then
		...
		if kbpscount == 16 then
			if VEM.ShaAssistStarModeChosed == "Dps" or VEM.ShaAssistStarModeChosed == "Healther" then
				...
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\end.mp3")
				end
			end
		end
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 119108 and self:AntiSpam(2, 3) then
		if not onPlatform then
			...
			if mod:IsDps() then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kdkjzz.mp3") --快打恐懼之子
			end
		end
	elseif spellId == 114936 and self:AntiSpam(5, 6) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_tenkj.mp3")
