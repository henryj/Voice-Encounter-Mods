...
function mod:OnCombatStart(delay)
	...
	if not mod:IsDps() then
		sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zbhx.ogg") --準備火息
	end
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(124967) and not phase2Started then--Phase 2 begin/Phase 1 end
		...
		if not mod:IsDps() then
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zbhx.ogg")
...
--		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\pthree.ogg") --P3
	elseif args:IsSpellID(117878) and args:IsPlayer() then
		...
		if (args.amount or 1) >= OCn and args.amount % 3 == 0 and self:IsInCombat() then--Warn every 3 stacks at 6 and above.
			...
			if not pthree then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_czgg.ogg") --超載過高
			end
		end
...
function mod:SPELL_AURA_REMOVED(args)
		...
		if warned == 2 then
			...
			self:Schedule(2, function()
				if not UnitDebuff("player", GetSpellInfo(117870)) then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kjzc.ogg") --快進中場
				end
			end)
		end
	elseif args:IsSpellID(132226) then
	...
	elseif args:IsSpellID(117878) and args:IsPlayer() and self:IsInCombat() then
		sndDD:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\didi.ogg") --~
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(117960) then
		...
		if not mod:IsDps() then
			if warned ~=2 then
				...
				sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zbhx.ogg")
			end
		end
	...
	elseif args:IsSpellID(129711) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zbbz.ogg") --準備爆炸
	...
	elseif args:IsSpellID(117949) then
		...
		if self:AntiSpam(3, 1) then
			sndCC:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_qssd.ogg")--驅散閃電
		end
	elseif args:IsSpellID(119358) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dqkd.ogg") --電球快打
...
function mod:RAID_BOSS_EMOTE(msg)
	if msg == L.Floor or msg:find(L.Floor) then
		if UnitDebuff("player", GetSpellInfo(117870)) then
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\leavecenter.ogg") --離開中場
		else
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zcxs.ogg") --中場即將消失
		end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 127362 and not warnfailed and self:AntiSpam(5, 3) then
		warnfailed = true
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\failed.ogg") --~
	end
end