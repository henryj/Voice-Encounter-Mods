...
local function warnPinnedDownTargets()
	...
	sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_yzjkd.mp3") --壓制箭快打
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwjh.mp3") --戰王激活
	sndWOP:Schedule(21, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3")
	if self:IsDifficulty("heroic10", "heroic25") then
		timerImperviousShieldCD:Start(40.7)
		warnImperviousShieldSoon:Schedule(35.7)
		sndDSA:Schedule(37.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.mp3") -- 戰王護盾準備
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(117837) then
		...
		if mod:CanRemoveEnrage() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\trannow.mp3") --快寧神
			sndWOP:Schedule(18, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjn.mp3")
		end
	elseif args:IsSpellID(117756) then
		warnCowardice:Show(args.destName)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_fsxt.mp3") --反傷形態
		Warned = false
		fanshang = true
	elseif args:IsSpellID(117737) then
		...
		if mengfirstime > 1 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dkxt.mp3") --癲狂形態
		end
		Warned = false
		fanshang = false
	...
	elseif args:IsSpellID(118303) then
		...
		elseif self:AntiSpam(10, 5) then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_hqcx.mp3") --黑球出現
		end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(117910) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3")
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bckd.mp3") --包抄快躲
		if qiangActive then
			timerFlankingOrdersCD:Start()
			sndWOP:Schedule(37, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3") --包抄準備
		else
			timerFlankingOrdersCD:Start(75)
			sndWOP:Schedule(72, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3")
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(118162) then
		...
		if args.sourceGUID == UnitGUID("target") then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stopatk.mp3") --注意停手
			VEM.Flash:Shake(1, 0, 0)
		end
		timerSleightOfHand:Start()
		timerSleightOfHandCD:Start()
		sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwhd.mp3") -- 盜王護盾準備
		...
		if mod.Options.optKZ == "KZ1" then
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kzc.mp3") --快製裁
		elseif mod.Options.optKZ == "KZ2" then
			sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kzc.mp3")
		end
	elseif args:IsSpellID(117506) then
		warnUndyingShadows:Show()
		timerUndyingShadowsCD:Start()
	...
	elseif args:IsSpellID(117697) then
		...
		sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.mp3") -- 巫王護盾準備
		self:Schedule(39, function()
		...
		elseif mod:IsMagicDispeller() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyhd.mp3") --注意护盾
			specWarnShieldOfDarkness:Show(args.sourceName)
			VEM.Flash:Shake(1, 0, 0)
		end
		if mod:IsMagicDispeller() then
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dispelnow.mp3") --快驅散
		end
	...
	elseif args:IsSpellID(117708) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\findmc.mp3") --注意心控
		if mengActive then
			if countxk == 1 then
				timerMaddeningShoutCD:Start()
				sndWOP:Schedule(40, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
			else
				timerMaddeningShoutCD:Start(50)
				sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
			end
		else
			timerMaddeningShoutCD:Start(77)
			sndWOP:Schedule(75, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
		end
...
--			countdownImperviousShield:Start(62)
			sndDSA:Schedule(59, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.mp3") -- 戰王護盾準備
...
--			countdownImperviousShield:Start(42)
			sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.mp3") -- 戰王護盾準備
			self:Schedule(39, function()
				if UnitName("target") == Qiang then
					specWarnDSoon:Show()
				end
		...
		if isDispellerZ then
			specWarnImperviousShield:Show(args.sourceName)
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dispelnow.mp3")
			VEM.Flash:Shake(1, 0, 0)
		...
		if (countzsb == 0 and timerAnnihilateCD:GetTime() < 6) or (countzsb >= 1 and timerAnnihilateCD:GetTime() < 27) then
			...
			if self:IsDifficulty("heroic10", "heroic25") then
				if mod:IsTank() then
					...
					specWarnQBH:Show()
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jmzb.mp3") -- 殲滅準備
				end
			end
		end
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 118088 and self:AntiSpam(2, 1) then--Volley
		...
		specWarnVolley:Show()
		timerVolleyCD:Start()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyjy.mp3") --注意箭雨
...
--	"<63.5 21:23:16> [UNIT_SPELLCAST_SUCCEEDED] Qiang the Merciless [[boss1:Cancel Activation::0:118219]]", -- [14068]
	elseif spellId == 118205 and self:AntiSpam(2, 3) then--Inactive Visual
		if UnitName(uId) == Zian then
			...
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.mp3")
		...
		elseif UnitName(uId) == Meng then
			...
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjn.mp3")
			timerMaddeningShoutCD:Start(30)--This boss retains Maddening Shout
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
			sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
		elseif UnitName(uId) == Qiang then
			qiangActive = false
			timerJL:Cancel()
			timerAnnihilateCD:Cancel()
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jmzb.mp3")
			timerImperviousShieldCD:Cancel()
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.mp3")
			warnImperviousShieldSoon:Cancel()
			timerFlankingOrdersCD:Start(30)--This boss retains Flanking Orders
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3")
			sndWOP:Schedule(27, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.mp3")
		elseif UnitName(uId) == Subetai then
			subetaiActive = false
			timerVolleyCD:Cancel()
			timerRainOfArrowsCD:Cancel()
			timerSleightOfHandCD:Cancel()
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwhd.mp3")
...
function mod:CHAT_MSG_MONSTER_YELL(msg, boss)
	...
	if boss == Zian then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwjh.mp3") --巫王激活
		...
		if self:IsDifficulty("heroic10", "heroic25") then
			...
			sndDSA:Schedule(37.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.mp3") -- 巫王護盾準備
			self:Schedule(37.5, function()
				if UnitName("target") == Zian then
					specWarnDSoon:Show()
				end
	...
	elseif boss == Meng then
		...
		if self:IsDifficulty("heroic10", "heroic25") then
			timerDeliriousCD:Start()
			if mod:CanRemoveEnrage() then
				sndWOP:Schedule(18, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjn.mp3") -- 狂王激怒準備
			end
			timerMaddeningShoutCD:Start(40)
			sndWOP:Schedule(35, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
		else
			timerMaddeningShoutCD:Start(20.5)
			sndWOP:Schedule(15, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.mp3")
		end
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjh.mp3") --狂王激活
		if mod.Options.optDD == "DD1" then
			specWarnDDL:Schedule(1)
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\interruptsoon.mp3") --打斷準備
		end
	...
	elseif boss == Subetai then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwjh.mp3") --盜王激活
		if self:IsDifficulty("heroic10", "heroic25") then
			timerSleightOfHandCD:Start(40.7)
			timerRainOfArrowsCD:Start(40)
			sndDSA:Schedule(37.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwhd.mp3") -- 盜王護盾準備
			self:Schedule(37.5, function()
				if UnitName("target") == Subetai then
					specWarnDSoon:Show()
				end
