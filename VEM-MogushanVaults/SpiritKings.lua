...
local function warnPinnedDownTargets()
	...
	sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_yzjkd.ogg") --壓制箭快打
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(117837) then
		...
		if mod:CanRemoveEnrage() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\trannow.ogg") --快寧神
			sndWOP:Schedule(18, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjn.ogg")
		end
	elseif args:IsSpellID(117756) then
		warnCowardice:Show(args.destName)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_fsxt.ogg") --反傷形態
		Warned = false
		fanshang = true
	elseif args:IsSpellID(117737) then
		...
		if mengfirstime > 1 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dkxt.ogg") --癲狂形態
		end
		Warned = false
		fanshang = false
	...
	elseif args:IsSpellID(118303) then
		...
		elseif self:AntiSpam(10, 5) then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_hqcx.ogg") --黑球出現
		end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(117910) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.ogg")
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bckd.ogg") --包抄快躲
		if qiangActive then
			timerFlankingOrdersCD:Start()
			sndWOP:Schedule(37, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.ogg") --包抄準備
		else
			timerFlankingOrdersCD:Start(75)
			sndWOP:Schedule(72, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.ogg")
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(118162) then
		...
		if args.sourceGUID == UnitGUID("target") then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stopatk.ogg") --注意停手
			VEM.Flash:Shake(1, 0, 0)
		end
		timerSleightOfHand:Start()
		timerSleightOfHandCD:Start()
		sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwhd.ogg") -- 盜王護盾準備
		...
		if mod.Options.optKZ == "KZ1" then
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kzc.ogg") --快製裁
		elseif mod.Options.optKZ == "KZ2" then
			sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kzc.ogg")
		end
	elseif args:IsSpellID(117506) then
		warnUndyingShadows:Show()
		timerUndyingShadowsCD:Start()
	...
	elseif args:IsSpellID(117697) then
		...
		sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.ogg") -- 巫王護盾準備
		self:Schedule(39, function()
		...
		elseif mod:IsMagicDispeller() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyhd.ogg") --注意护盾
			specWarnShieldOfDarkness:Show(args.sourceName)
			VEM.Flash:Shake(1, 0, 0)
		end
		if mod:IsMagicDispeller() then
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dispelnow.ogg") --快驅散
		end
	...
	elseif args:IsSpellID(117708) then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\findmc.ogg") --注意心控
		if mengActive then
			if countxk == 1 then
				timerMaddeningShoutCD:Start()
				sndWOP:Schedule(40, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
			else
				timerMaddeningShoutCD:Start(50)
				sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
			end
		else
			timerMaddeningShoutCD:Start(77)
			sndWOP:Schedule(75, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
		end
...
--			countdownImperviousShield:Start(62)
			sndDSA:Schedule(59, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.ogg") -- 戰王護盾準備
...
--			countdownImperviousShield:Start(42)
			sndDSA:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.ogg") -- 戰王護盾準備
			self:Schedule(39, function()
				if UnitName("target") == Qiang then
					specWarnDSoon:Show()
				end
		...
		if isDispellerZ then
			specWarnImperviousShield:Show(args.sourceName)
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dispelnow.ogg")
			VEM.Flash:Shake(1, 0, 0)
		...
		if (countzsb == 0 and timerAnnihilateCD:GetTime() < 6) or (countzsb >= 1 and timerAnnihilateCD:GetTime() < 27) then
			...
			if self:IsMythic() then
				if mod:IsTank() then
					...
					specWarnQBH:Show()
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jmzb.ogg") -- 殲滅準備
				end
			end
		end
	end
...
--	"<63.5 21:23:16> [UNIT_SPELLCAST_SUCCEEDED] Qiang the Merciless [[boss1:Cancel Activation::0:118219]]", -- [14068]
	elseif spellId == 118205 and self:AntiSpam(2, 3) then--Inactive Visual
		if UnitName(uId) == Zian then
			...
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.ogg")
		...
		elseif UnitName(uId) == Meng then
			...
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjn.ogg")
			timerMaddeningShoutCD:Start(30)--This boss retains Maddening Shout
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
			sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
		elseif UnitName(uId) == Qiang then
			qiangActive = false
			timerJL:Cancel()
			timerAnnihilateCD:Cancel()
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jmzb.ogg")
			timerImperviousShieldCD:Cancel()
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zwhd.ogg")
			warnImperviousShieldSoon:Cancel()
			timerFlankingOrdersCD:Start(30)--This boss retains Flanking Orders
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.ogg")
			sndWOP:Schedule(27, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_bczb.ogg")
		elseif UnitName(uId) == Subetai then
			subetaiActive = false
			timerVolleyCD:Cancel()
			timerRainOfArrowsCD:Cancel()
			timerSleightOfHandCD:Cancel()
			sndDSA:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dwhd.ogg")
...
function mod:CHAT_MSG_MONSTER_YELL(msg, boss)
	...
	if boss == Zian then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwjh.ogg") --巫王激活
		...
		if self:IsMythic() then
			...
			sndDSA:Schedule(37.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwhd.ogg") -- 巫王護盾準備
			self:Schedule(37.5, function()
				if UnitName("target") == Zian then
					specWarnDSoon:Show()
				end
	...
	elseif boss == Meng then
		...
		if self:IsMythic() then
			...
			sndWOP:Schedule(35, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
		else
			timerMaddeningShoutCD:Start(20.5)
			sndWOP:Schedule(15, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_jknh.ogg")
		end
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kwjh.ogg") --狂王激活
		if mod.Options.optDD == "DD1" then
			specWarnDDL:Schedule(1)
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\interruptsoon.ogg") --打斷準備
		end
