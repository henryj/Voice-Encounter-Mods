local mod	= VEM:NewMod(682, "VEM-MogushanVaults", nil, 317)
...
do
	...
	function mod:SetVoodooIcons()
		...
		for i, v in ipairs(voodooDollTargetIcons) do
			-- VEM:SetIcon() is used because of follow reasons
			--1. It checks to make sure you're on latest vem version, if you are not, it disables icon setting so you don't screw up icons (ie example, a newer version of mod does icons differently)
			--2. It checks global vem option "DontSetIcons"
...
function mod:SPELL_AURA_APPLIED(args)--We don't use spell cast success for actual debuff on >player< warnings since it has a chance to be resisted.
	if args:IsSpellID(122151) then
		if args:IsPlayer() and (not mod:IsTank()) then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_wwsn.mp3")--娃娃是你
		end
	...
	elseif args:IsSpellID(116161, 116160) then -- 116161 is normal and heroic, 116160 is lfr.
		if args:IsPlayer() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kj.mp3")--跨界
			warnSuicide:Schedule(25)
--			countdownCrossedOver:Start(29)
			timerCrossedOver:Start(29)
			sndWOP:Schedule(23.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jjsw.mp3")--即將死亡
			sndWOP:Schedule(25, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
	...
	elseif args:IsSpellID(116278) then--this is tank spell, no delays?
		if args:IsPlayer() then--no latency check for personal notice you aren't syncing.
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kj.mp3")
			timerSoulSever:Start()
--			countdownCrossedOver:Start(29)
			warnSuicide:Schedule(25)
			sndWOP:Schedule(23.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jjsw.mp3")
			sndWOP:Schedule(25, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(26, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(27, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(28, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
	...
	elseif args:IsSpellID(117752) then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kl.mp3") --狂亂
	end
	if UnitDebuff("player", GetSpellInfo(122181)) then
		if not inTotem then
			specWarnGD:Show()
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jrgd.mp3") --進入管道
		end
		inTotem = true
	else
		inTotem = false
...
--		countdownCrossedOver:Cancel()
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kj.mp3")
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jjsw.mp3")
...
function mod:OnSync(msg, guid)
	...
	if msg == "SummonTotem" and self:AntiSpam(10, 3) then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lhtt.mp3")
		
		if self.Options.countTT == "count4" then
			if totemn % 4 == 1 then
				sndWOP:Schedule(1.2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			...
			elseif totemn % 4 == 3 then
				sndWOP:Schedule(1.2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			else
				sndWOP:Schedule(1.2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			end
	...
	elseif msg == "VoodooTargets" and guids[guid] then
		...
		if self.Options.SetIconOnVoodoo then
			table.insert(voodooDollTargetIcons, VEM:GetRaidUnitId(guids[guid]))
	...
	elseif msg == "VoodooGoneTargets" and guids[guid] and self.Options.SetIconOnVoodoo then
		removeIcon(VEM:GetRaidUnitId(guids[guid]))
	...
	elseif msg == "Gointotem" and guid then
		if self.Options.GoTotemClient then
			...
			if guid == UnitName("player") then
				specWarninTT:Show()
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_zyjc.mp3")--注意進場
			end
		end
	end
end
