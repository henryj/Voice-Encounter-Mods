local mod	= VEM:NewMod(1147, "VEM-BlackrockFoundry", nil, 457)
...
function mod:CHAT_MSG_MONSTER_YELL(msg, npc, _, _, target)
	if target == Train then
		...
		if self:IsMythic() then
			if count >= 12 then
				print("Train Set: "..count..". VEM has no train data beyond this point. Write down lane(s) trains come from in 5 seconds with train set number and give it to us")
				return
			end
