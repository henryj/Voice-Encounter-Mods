local mod	= VEM:NewMod(1122, "VEM-BlackrockFoundry", nil, 457)
...
local function updateBeasts(cid, status, beastName)
	if VEM.BossHealth:IsShown() then
		if status == 3 then--Add Boss, keep Beast
			VEM.BossHealth:AddBoss(76865, L.name)
		elseif status == 2 then--Just Remove Beast
			VEM.BossHealth:AddBoss(cid, beastName)
		elseif status == 1 then--Add beast, remove boss
			VEM.BossHealth:RemoveBoss(76865)
			VEM.BossHealth:AddBoss(cid, beastName)
			...
			if cid == 76884 or cid == 76874 or cid == 76945 or cid == 76946 then
				...
				if cid == 76884 then--Cruelfang
					...
					if self.Options.RangeFrame and not self.vb.RylakAbilities then
						VEM.RangeCheck:Show(7)--Upgrade range frame to 7 now that he has rend and tear. TODO: If this attack doesn't target melee
					end
				...
				elseif cid == 76874 then--Dreadwing
					...
					if self.Options.RangeFrame then
						VEM.RangeCheck:Show(8)--Update range frame to 8 for Scrapnal. TODO, again, see if melee affected by this or not
					end
					--Cancel timers for abilities he can't use from other dead beasts
