local mod	= VEM:NewMod(1128, "VEM-Highmaul", nil, 477)
...
local specWarnBerserkerRush			= mod:NewSpecialWarningMoveTo(158986, nil, VEM_CORE_AUTO_SPEC_WARN_OPTIONS.run:format(158986), nil, 3)--Creative use of warning. Run option text but a moveto warning to get players in LFR to actually run to the flame jet instead of being clueless.
...
local specWarnOnTheHunt				= mod:NewSpecialWarningMoveTo(162497, nil, VEM_CORE_AUTO_SPEC_WARN_OPTIONS.run:format(162497))--Does not need yell, tigers don't cleave other targets like berserker rush does.
...
function mod:OnCombatStart(delay)
	...
	if self.Options.RangeFrame then
		VEM.RangeCheck:Show(4)--For Mauling Brew splash damage.
	end
