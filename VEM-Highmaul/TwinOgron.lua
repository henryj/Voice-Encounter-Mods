local mod	= VEM:NewMod(1148, "VEM-Highmaul", nil, 477)
...
function mod:SPELL_CAST_START(args)
	...
	if spellId == 158057 then
		...
		if (self.vb.QuakeCount+1) % 2 == 0 then--Even ones have longer Cd than odds
			...
			VEM:Debug("Activating Quake Delay, next quake is an even one")
		else
	...
	elseif spellId == 157943 then
		...
		if timeRemaining < 60 and timeRemaining > 45 then--Quake failsafe will activate and delay Whirlwind
			...
			VEM:Debug("Activating Whirlwind Delay, quake is less than 50 seconds away")
		else
