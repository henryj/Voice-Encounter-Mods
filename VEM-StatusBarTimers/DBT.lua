...
do
	...
	local function delaySkinCheck(self)
		...
		if not skins then--Returns nil if checked too soon
			VEM:Schedule(3, delaySkinCheck, self)
			return
		end
		local enabled = GetAddOnEnableState(UnitName("player"), "VEM-DefaultSkin")
	...
	function DBT:LoadOptions(id)
		-- recover old options (VEM_SavedOptions) if possible (saved by VEM, before DBT was a separate addon)
		...
		VEM:Schedule(2, delaySkinCheck, self)
	end
end
...
function DBT:GetBarIterator()
	if not self.bars then
		VEM:Debug("GetBarIterator failed for unknown reasons")
		return
	end
...
function DBT:ApplyStyle()
	...
	if applyFailed then
		applyFailed = false
		VEM:AddMsg(VEM_CORE_LOAD_SKIN_COMBAT)
	end
end
