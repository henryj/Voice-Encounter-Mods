			...
			if min and max then
				if min <= 8 then
					if mod:AntiSpam(1, 6) then
						...
						sndWSP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stilldanger.mp3") --危險
					end
				elseif min <= 15 then
					if mod:AntiSpam(4, 5) then
						VEM.Flash:Shake(1, 0, 0)
						sndWSP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killspirit.mp3") --靈魂快打
					end
--				elseif min >= 60 then
--					if mod:AntiSpam(4, 5) then
--						sndWSP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\safenow.mp3")
--					end
				end
			end	
		end
...
function mod:OnCombatStart(delay)
	...
	self:Schedule(9, function()
		if self:IsTanking("player", "boss1") or mod.Options.ccsoon then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\watchimpale.mp3")
		end
	end)
	...
	if self:IsDifficulty("heroic10", "heroic25") then
		...
		if mod:IsHealer() then
			if select(2, UnitClass("player")) == "PRIEST" then
				sndWOP:Schedule(42, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_esnh.mp3")
			end
			sndWOP:Schedule(57, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmhn.mp3")
		end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 136741 then--Regular double swipe
		...
		sndDB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_scsj.mp3") --雙重掃擊
	...
	elseif args.spellId == 136770 then--Double swipe that follows a charge (136769)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_scsj.mp3")
		warnDoubleSwipe:Show()
		specWarnDoubleSwipe:Show()
	...
	elseif args.spellId == 137458 then
		...
		nhcount = nhcount + 1		
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --準備AE
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 136487 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdtt.mp3")--閃電圖騰
	...
	elseif args.spellId == 136564 then
		if self:AntiSpam(2, 7) then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_bqxx.mp3") --冰球小心
		end
	end
end
--BH ADD END
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 136817 then
		...
		if MyJS() then
			...
			sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --準備AE
		end
	...
	elseif args.spellId == 136821 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_hldn.mp3")--哈里登暴怒
	...
	elseif args.spellId == 137237 then
		...
		sndOrb:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_ksbz.mp3") --控獸寶珠
...
--BH DELETE		soundDireFixate:Play()
			-- BH ADD
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_lhsd.mp3")
			sndWOP:Schedule(1,"Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_lhsd.mp3")
			checksprange()
			-- BH ADD END
		end
	elseif args.spellId == 136512 then
		if args:IsPlayer() then
			specWarnHex:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stopattack.mp3") -- 停止攻擊
	...
	elseif args.spellId == 137294 then
		...
		sndWOP:Schedule(9.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		if closedoor == 1 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_smgb.mp3")--沙門關閉
		elseif closedoor == 2 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_dmgb.mp3")--毒門關閉
		elseif closedoor == 3 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_bmgb.mp3")--冰門關閉
		end
	elseif args.spellId ==  136670 then
		if args:IsPlayer() then
			if (args.amount or 1) >= 2 then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dshigh.mp3")--致死過高
			end
		end
	-- BH ADD END
	end
...
function mod:INSTANCE_ENCOUNTER_ENGAGE_UNIT(event)
	if UnitExists("boss2") and tonumber(UnitGUID("boss2"):sub(6, 10), 16) == 69374 and not jalakEngaged then--Jalak is jumping down
		...
		-- BH ADD
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_zscz.mp3") --戰神參戰
		if MyJS() then
			specWarnJSA:Schedule(2)
			sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
		else
			sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --準備AE
		end
...
function mod:UNIT_DIED(args)
	...
	if cid == 69374 then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
...
function mod:OnSync(msg, target)
	if msg == "ChargeTo" and target then
		...
		if target == UnitName("player") then
			...
			yellCharge:Yell()
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_nbcf.mp3")
			sndWOP:Schedule(0.5,"Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_nbcf.mp3")
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_cfkd.mp3") --衝鋒快躲
		end
	...
	--So it goes, door, 18.91 seconds later, 1 add jumps down. 18.91 seconds later, next 2 drop down. 18.91 seconds later, dinomancer drops down, then 56.75 seconds later, next door starts.
		...
		if mod:IsDps() then
			sndWOP:Schedule(56.75, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_ylsd.mp3") --禦龍師快打
		else
			sndWOP:Schedule(56.75, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_ylsc.mp3") --禦龍師出現
		end
		if doorNumber == 1 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_smkq.mp3") --沙門開啟
			timerAdds:Start(18.9, Farraki)
			warnAdds:Schedule(18.9, Farraki)
			self:Schedule(18.9, addsDelay, Farraki)
		elseif doorNumber == 2 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_dmkq.mp3") --毒门開啟
		...
		elseif doorNumber == 3 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_bmkq.mp3") --冰门開啟
		...
		elseif doorNumber == 4 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_xmkq.mp3") --熊門開啟
	...
	elseif msg == "Duye" then
		if self:AntiSpam(2, 10) then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_zyfs.mp3") --注意分身
		end
	end
end
