...
function mod:SPELL_CAST_START(args)
	if args.spellId == 135095 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_yllj.ogg") --遠離雷擊
		if phase < 3 then
			timerThunderstruckCD:Start()
		else
			timerThunderstruckCD:Start(30)
			specWarnThunderstruckSoon:Schedule(25)
			sndWOP:Schedule(25, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmlj.ogg") --5秒後雷擊
	...
	elseif args.spellId == 136850 then
		...
		VEM.Flash:Shake(1, 0, 0)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdb.ogg") --閃電鞭
	elseif args.spellId == 136478 then
		...
		elseif mod:IsTank() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jlz.ogg") --巨雷斬
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(135000, 134912) then--Is 135000 still used on 10 man?
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_zs.ogg")--斬首快跑
		else
	...
	elseif args.spellId == 135695 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countseven.ogg")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.ogg")
			sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.ogg")
			sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
			sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
			sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
			sndWOP:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		else
			...
			if uId then
				...
				if inRange and inRange < 50 then
					...
					if self:AntiSpam(3, 11) then
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jhfd.ogg") --集合分擔
						sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countseven.ogg")
						sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.ogg")
						sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.ogg")
						sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
	...
	elseif args.spellId == 136295 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\targetyou.ogg") --目標是你
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.ogg")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
				...
				if inRange and inRange < 50 then
					...
					if self:AntiSpam(3, 6) then
						if intermissionActive and (firstchain == 1) then
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_hmzb.ogg") --昏迷圈準備
							twocirle = true
							circletime = GetTime()
						else
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_czjh.ogg") --超載集合
	...
	elseif args.spellId == 135681 and args:GetDestCreatureID() == 68397 then--East (Diffusion Chain)
		...
		if phase > 1 then
			if self:AntiSpam(10, 20) then
				...
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.ogg")--閃電鏈準備
	...
	elseif args.spellId == 139011 then
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jtzy.ogg") --注意擊退
		else
			local uId = VEM:GetRaidUnitId(args.destName)
			if uId then
				local x, y = GetPlayerMapPosition(uId)
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 135991 then
		...
		if not intermissionActive then
			...
			sndWOP:Schedule(36, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.ogg")
			if phase > 1 then
		...
		if intermissionActive and self:IsMythic() then
			if firstchain == 0 then
				...
				if twocirle then
					...
					sndWOP:Schedule(1.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
					sndWOP:Schedule(2.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
					sndWOP:Schedule(3.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
					sndWOP:Schedule(4.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
				end
			end
		end
	elseif args.spellId == 136543 and self:AntiSpam(2, 1) then
		...
		if phase < 3 then
			...
			sndWOP:Schedule(40, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.ogg") --5秒後電球
		else
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 135681 and args:GetDestCreatureID() == 68397 and not intermissionActive then--East (Diffusion Chain)
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.ogg")
		if phase > 1 then
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, _, _, _, target)
	if msg:find("spell:137176") then--Overloaded Circuits (Intermission ending and next phase beginning)
		...
		if msg:find("spell:135680") then--North (Static Shock)
			northDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jdsh.ogg") --靜電震擊損毀
		elseif msg:find("spell:135681") then--East (Diffusion Chain)
			eastDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sssh.ogg") --散射電練損毀
		elseif msg:find("spell:135682") then--South (Overcharge)
			southDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_czsh.ogg") --電能超載損毀
		...
		if self:IsMythic() then
			...
			if eastDestroyed then
				...
				sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.ogg")
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.ogg")
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\pthree.ogg")
		if phase == 2 then--Start Phase 2 timers
			warnPhase2:Show()
			lightp2count = 0
			timerSummonBallLightningCD:Start(15)
			sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.ogg") --5秒後電球
			timerLightningWhipCD:Start(30)
		...
		elseif phase == 3 then--Start Phase 3 timers
			...
			sndWOP:Schedule(17, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_qfzb.ogg") --強風準備
			timerLightningWhipCD:Start(21.5)
			timerThunderstruckCD:Start(36)
			specWarnThunderstruckSoon:Schedule(31)
			sndWOP:Schedule(31, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmlj.ogg") --5秒後雷擊
...
local function LoopIntermission()
	...
	if not eastDestroyed then
		if mod:IsLFR() then
			...
			sndWOP:Schedule(4, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.ogg")--注意分散
		end
	end
	if not westDestroyed then
		if mod:IsLFR() then
			...
			sndWOP:Schedule(9, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_fscq.ogg")
		else
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 137146 and self:AntiSpam(2, 2) then--Supercharge Conduits (comes earlier than other events so we use this one)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_cjcn.ogg") --超級充能		
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.ogg")
		if phase == 1 then
			sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.ogg")--2階段準備
		elseif phase == 2 then
			sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\pthree.ogg")--3階段準備
		end
		...
		if not eastDestroyed then
			if self:IsLFR() then
				...
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.ogg")--注意分散
			end
		end
		if self.Options.RangeFrame then
			VEM.RangeCheck:Show(8)
	...
	elseif spellId == 136395 and self:AntiSpam(2, 3) and not intermissionActive then--Bouncing Bolt (During intermission phases, it fires randomly, use scheduler and filter this :\)
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_fscq.ogg")
	...
	elseif spellId == 136869 and self:AntiSpam(2, 4) then--Violent Gale Winds
		...
		sndWOP:Schedule(27, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_qfzb.ogg") --強風準備
	...
	if msg == "Refle" then
		if self:AntiSpam(20, 15) then
			...
			if ((mod.Options.optDD == "DD1") and (chaincount % 2 == 1)) or ((mod.Options.optDD == "DD2") and (chaincount % 2 == 0)) then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_kkdf.ogg")
			end
		end
	end
end