...
function mod:SPELL_CAST_START(args)
	if args.spellId == 135095 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_yllj.mp3") --遠離雷擊
		if phase < 3 then
			timerThunderstruckCD:Start()
		else
			timerThunderstruckCD:Start(30)
			specWarnThunderstruckSoon:Schedule(25)
			sndWOP:Schedule(25, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmlj.mp3") --5秒後雷擊
	...
	elseif args.spellId == 136850 then
		...
		VEM.Flash:Shake(1, 0, 0)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdb.mp3") --閃電鞭
	elseif args.spellId == 136478 then
		...
		elseif mod:IsTank() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jlz.mp3") --巨雷斬
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(135000, 134912) then--Is 135000 still used on 10 man?
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_zs.mp3")--斬首快跑
		else
	...
	elseif args.spellId == 135695 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
			sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		else
			...
			if uId then
				...
				if inRange and inRange < 50 then
					...
					if self:AntiSpam(3, 11) then
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jhfd.mp3") --集合分擔
						sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
						sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
						sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
						sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
	...
	elseif args.spellId == 136295 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\targetyou.mp3") --目標是你
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
				...
				if inRange and inRange < 50 then
					...
					if self:AntiSpam(3, 6) then
						if intermissionActive and (firstchain == 1) then
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_hmzb.mp3") --昏迷圈準備
							twocirle = true
							circletime = GetTime()
						else
							sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_czjh.mp3") --超載集合
							sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
							sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
							sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
							sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
						end						
					end
	...
	elseif args.spellId == 135681 and args:GetDestCreatureID() == 68397 then--East (Diffusion Chain)
		...
		if phase > 1 then
			if self:AntiSpam(10, 20) then
				...
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.mp3")--閃電鏈準備
	...
	elseif args.spellId == 139011 then
		...
		if args:IsPlayer() then
			...
			VEM.Flash:Shake(1, 0, 0)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jtzy.mp3") --注意擊退
		else
			local uId = VEM:GetRaidUnitId(args.destName)
			if uId then
				local x, y = GetPlayerMapPosition(uId)
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 135991 then
		...
		if not intermissionActive then
			...
			sndWOP:Schedule(36, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.mp3")
			if phase > 1 then
		...
		if intermissionActive and self:IsDifficulty("heroic10", "heroic25") then
			if firstchain == 0 then
				...
				if twocirle then
					...
					sndWOP:Schedule(1.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
					sndWOP:Schedule(2.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
					sndWOP:Schedule(3.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
					sndWOP:Schedule(4.5-circletime, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
				end
			end
		end
	elseif args.spellId == 136543 and self:AntiSpam(2, 1) then
		...
		if phase < 3 then
			...
			sndWOP:Schedule(40, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.mp3") --5秒後電球
		else
			timerSummonBallLightningCD:Start(30)
			sndWOP:Schedule(25, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.mp3")
		end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 135681 and args:GetDestCreatureID() == 68397 and not intermissionActive then--East (Diffusion Chain)
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.mp3")
		if phase > 1 then
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, _, _, _, target)
	if msg:find("spell:137176") then--Overloaded Circuits (Intermission ending and next phase beginning)
		...
		if msg:find("spell:135680") then--North (Static Shock)
			northDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_jdsh.mp3") --靜電震擊損毀
		elseif msg:find("spell:135681") then--East (Diffusion Chain)
			eastDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sssh.mp3") --散射電練損毀
		elseif msg:find("spell:135682") then--South (Overcharge)
			southDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_czsh.mp3") --電能超載損毀
		elseif msg:find("spell:135683") then--West (Bouncing Bolt)
			westDestroyed = true
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_ttsh.mp3") --彈跳閃電損毀
		end
		if self:IsDifficulty("heroic10", "heroic25") then
			...
			if eastDestroyed then
				...
				sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdls.mp3")
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3")
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\pthree.mp3")
		if phase == 2 then--Start Phase 2 timers
			warnPhase2:Show()
			lightp2count = 0
			timerSummonBallLightningCD:Start(15)
			sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.mp3") --5秒後電球
			timerLightningWhipCD:Start(30)
		...
		elseif phase == 3 then--Start Phase 3 timers
			...
			sndWOP:Schedule(17, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_qfzb.mp3") --強風準備
			timerLightningWhipCD:Start(21.5)
			timerThunderstruckCD:Start(36)
			specWarnThunderstruckSoon:Schedule(31)
			sndWOP:Schedule(31, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmlj.mp3") --5秒後雷擊
			timerSummonBallLightningCD:Start(41.5)
			sndWOP:Schedule(36, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.mp3") --5秒後電球
			if self.Options.RangeFrame then
				if self:IsDifficulty("heroic10", "heroic25") then
...
local function LoopIntermission()
	...
	if not eastDestroyed then
		if mod:IsDifficulty("lfr25") then
			...
			sndWOP:Schedule(13.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")--注意分散
		else
			timerDiffusionChainCD:Start(8)
			specWarnDiffusionChainSoon:Schedule(4)
			sndWOP:Schedule(4, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")--注意分散
		end
	end
	if not westDestroyed then
		if mod:IsDifficulty("lfr25") then
			...
			sndWOP:Schedule(9, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_fscq.mp3")
		else
			warnBouncingBolt:Schedule(14)
			specWarnBouncingBolt:Schedule(14)
			timerBouncingBoltCD:Start(14)
			sndWOP:Schedule(14, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_fscq.mp3")
		end
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 137146 and self:AntiSpam(2, 2) then--Supercharge Conduits (comes earlier than other events so we use this one)
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_cjcn.mp3") --超級充能		
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_wmdq.mp3")
		if phase == 1 then
			sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3")--2階段準備
		elseif phase == 2 then
			sndWOP:Schedule(45, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\pthree.mp3")--3階段準備
		end
		...
		if not eastDestroyed then
			if self:IsDifficulty("lfr25") then
				...
				sndWOP:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")--注意分散
			else
				timerDiffusionChainCD:Start(6)
				specWarnDiffusionChainSoon:Schedule(2)
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")--注意分散
			end
		end
		if self.Options.RangeFrame then
			VEM.RangeCheck:Show(8)
	...
	elseif spellId == 136395 and self:AntiSpam(2, 3) and not intermissionActive then--Bouncing Bolt (During intermission phases, it fires randomly, use scheduler and filter this :\)
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_fscq.mp3")
	...
	elseif spellId == 136869 and self:AntiSpam(2, 4) then--Violent Gale Winds
		...
		sndWOP:Schedule(27, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_qfzb.mp3") --強風準備
	...
	if msg == "Refle" then
		if self:AntiSpam(20, 15) then
			...
			if ((mod.Options.optDD == "DD1") and (chaincount % 2 == 1)) or ((mod.Options.optDD == "DD2") and (chaincount % 2 == 0)) then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_tt_kkdf.mp3")
			end
		end
	end
end