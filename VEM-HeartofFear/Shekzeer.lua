...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(123707) then
		...
		if args:IsPlayer() and (args.amount or 1) >= 4 then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_csgg.ogg") --層數過高
		else
	...
	elseif args:IsSpellID(123788) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kjyy.ogg") --快進音域
		end
	...
	elseif args:IsSpellID(124862) then
		...
		if self:AntiSpam(5, 3) then
			...
			if MyTT() then
				...
				sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zlzb.ogg") --戰慄準備
				sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
				sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
				sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
			end
		...
		if mod:IsHealer() then
			sndWOP:Schedule(4, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\dispelnow.ogg")
		end
	...
	elseif args:IsSpellID(124097) then
		if args:IsPlayer() and self:AntiSpam(5, 2) then --prevent spam in heroic
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_szkp.ogg") --樹脂
		end
	...
	elseif args:IsSpellID(123845) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_lxkp.ogg")--連線快跑
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(125826) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_xjwc.ogg") --陷阱完成
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(124849) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_kjts.ogg")--恐懼吞噬
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 125098 then--Yell is about 1.5 seconds faster then this event, BUT, it also requires localizing. I don't think doing it this way hurts anything.
		...
		sndWOP:Schedule(152, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_nwzb.ogg") --女王準備
		sndWOP:Schedule(153, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
		sndWOP:Schedule(154, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
		sndWOP:Schedule(155, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
		sndWOP:Schedule(156, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		if self.Options.InfoFrame then
	...
	elseif spellId == 125304 then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_nwzb.ogg")
		...
		ptwo = false
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\phasechange.ogg")--階段轉換
		sndWOP:Schedule(147, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.ogg")--P2
		sndWOP:Schedule(148, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
		sndWOP:Schedule(149, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
		sndWOP:Schedule(150, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
		sndWOP:Schedule(151, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		...
		if UnitDebuff("player", GetSpellInfo(123184)) then
			if self:IsMythic() then
				...
				if UnitHealth(uId) / UnitHealthMax(uId) <= 0.5 and not warnedhp then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\checkhp.ogg")
					warnedhp = true
				end
			end
		else
...
function mod:OnSync(msg, guid, hp)
	if msg == "ybhealth" and guid and hp then
		...
		if warnhp < 11 and not warnedLowHP[guid] then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ybbz.ogg") --音波爆炸準備
		end
		if warnhp == 5  and not warnedLowHP5[guid] then
			warnedLowHP5[guid] = true
			sndYB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.ogg")
		end
		if warnhp == 4  and not warnedLowHP4[guid] then
			warnedLowHP4[guid] = true
			sndYB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
		end
		if warnhp == 3  and not warnedLowHP3[guid] then
			warnedLowHP3[guid] = true
			sndYB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
		end
		if warnhp == 2  and not warnedLowHP2[guid] then
			warnedLowHP2[guid] = true
			sndYB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
		end
		if warnhp == 1  and not warnedLowHP1[guid] then
			warnedLowHP1[guid] = true
			sndYB:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		end
...
function mod:RAID_BOSS_EMOTE(msg)
	if msg:find("spell:126121") then
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_yyfh.ogg") --音域腐化
	end
end