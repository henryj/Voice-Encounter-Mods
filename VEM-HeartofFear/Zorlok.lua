...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(122713) then
		...
		if mod:IsHealer() then
			sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\healall.mp3") --注意群療
		end
	...
	elseif args:IsSpellID(122761) and self:AntiSpam(2, 1) then
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_tqzb.mp3") --吐氣準備
	...
	elseif args:IsSpellID(122474, 122496, 123721, 122513) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_zzyb.mp3") --左轉音波
	...
	elseif args:IsSpellID(122479, 122497, 123722, 122514) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_yzyb.mp3") --右轉音波
		timerAttenuation:Start()
		if platform < 4 then
			timerAttenuationCD:Start()
		else
...
function mod:OnSync(msg)
	if msg == "ForceandVerve" then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_dyq.mp3") --快進定音區
