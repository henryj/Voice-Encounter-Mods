...
DBM_CORE_NEED_SUPPORT				= "你是否擁有良好的程式開發或語言能力? 如果是的話, DBM團隊真的需要你的幫助以保持成為WOW裡最佳的首領模組。觀看 http://bbs.ngacn.cc/read.php?tid=5397726&page=1 或 https://github.com/henryj/Voice-Encounter-Mods 來加入團隊。"
DBM_HOW_TO_USE_MOD					= "歡迎使用黑手之鄉VEM-語音版。在聊天頻道輸入 /vem 打開設定開始設定。你可以載入特定區域後為任何首領設定你喜歡的特別設置。VEM會在第一次啟動時嘗試掃描你的職業天賦，但有些選項你可能想打開。"
...
DBM_CORE_LOAD_MOD_SUCCESS			= "成功載入%s模組。輸入/vem或/vem help有更多選項。"
...
DBM_CORE_BAD_LOAD					= "VEM偵測到你的此副本的模組在戰鬥中讀取失敗。一旦脫離戰鬥，請立即輸入/consoel reloadui重新載入。"

DBM_CORE_LOOT_SPEC_REMINDER			= "你目前的專精為:%s。而你目前的拾取選擇為:%s。"

DBM_CORE_BIGWIGS_ICON_CONFLICT		= "VEM偵測到你同時開啟BigWigs和VEM的團隊圖示。請關閉其中之一的團隊圖示功能以免與你的隊伍隊長產生衝突。"
...
DBM_CORE_VERSIONCHECK_HEADER		= "Voice Encounter Mods - 版本檢測"
DBM_CORE_VERSIONCHECK_ENTRY			= "%s：%s(r%d)"
DBM_CORE_VERSIONCHECK_ENTRY_TWO		= "%s: %s (r%d) & %s (r%d)"
DBM_CORE_VERSIONCHECK_ENTRY_NO_DBM	= "%s：尚未安裝任何團隊首領模組"
DBM_CORE_VERSIONCHECK_FOOTER			= "團隊中有%d玩家正在使用VEM|DBM / %d玩家正在使用Bigwigs"
DBM_CORE_YOUR_VERSION_OUTDATED			= "你的 Voice Encounter Mods 已經過期。請到 https://github.com/henryj/Voice-Encounter-Mods 下載最新版本。"
--DBM_BIG_WIGS (Same as English locales)
--DBM_BIG_WIGS_ALPHA (Same as English locales)

DBM_CORE_UPDATEREMINDER_HEADER			= "你的Voice Encounter Mods已經過期。\n你可以在此網址下載到新版本%s(r%d)："
DBM_CORE_UPDATEREMINDER_HEADER_ALPHA	= "你的alpha版本Voice Encounter Mods已經過期。\n 你至少落後%d個測試版本。建議VEM使用者使用最新的alpha或最新的穩定版本。過期的alpha版本可能會有低落或未完成的功能。"
...
DBM_CORE_UPDATEREMINDER_DISABLE			= "警告: 你的VEM版本已大幅度的過期(%d修訂版)，已禁用除非更新。這是為了確保舊而不相容的程式碼不會對你而團隊夥伴造成低落的遊戲體驗。"
...
DBM_PIZZA_ERROR_USAGE				= "命令：/vem [broadcast] timer <時間（秒）> <文字>"
...
DBM_ARROW_NO_RAIDGROUP				= "VEM箭頭僅可在副本中調試。"
...
DBM_INSTANCE_INFO_STATUS_UPDATE		= "從%d個玩家獲得訊息，來自%d個VEM|DBM用戶：%d人發送了資料, %d人拒絕回傳資料。繼續為更多回覆等待%d秒..."
...
DBM_INSTANCE_INFO_STATS_NO_RESPONSE	= "沒有安裝最新版本的VEM:%s"
...
DBM_CORE_LAG_HEADER					= "Voice Encounter Mods - 延遲檢測"
