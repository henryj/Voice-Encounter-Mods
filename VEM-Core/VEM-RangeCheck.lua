...
function createTextFrame()
	local elapsed = 0
	local textFrame = CreateFrame("GameTooltip", "VEMRangeCheck", UIParent, "GameTooltipTemplate")
	dropdownFrame = CreateFrame("Frame", "VEMRangeCheckDropdown", textFrame, "UIDropDownMenuTemplate")
	textFrame:SetFrameStrata("DIALOG")
	textFrame:SetPoint(VEM.Options.RangeFramePoint, UIParent, VEM.Options.RangeFramePoint, VEM.Options.RangeFrameX, VEM.Options.RangeFrameY)
...
do
	...
	function updateRangeFrame()
		...
		if tEnabled then
			...
			textFrame:SetText(VEM_CORE_RANGECHECK_HEADER:format(activeRange), 1, 1, 1)
		end
		if rEnabled and prevRange ~= activeRange then
			...
			radarFrame.text:SetText(VEM_CORE_RANGERADAR_HEADER:format(activeRange))
		end
		...
		if rEnabled then
			if prevNumClosePlayer ~= closePlayer then
				radarFrame.inRangeText:SetText(VEM_CORE_RANGERADAR_IN_RANGE_TEXT:format(closePlayer))
...
function rangeCheck:Show(range, filter, forceshow, redCircleNumPlayers)
	if (not IsInGroup() or VEM.Options.DontShowRangeFrame) and not forceshow then return end
	...
	if VEM.Options.RangeFrameFrames == "text" or VEM.Options.RangeFrameFrames == "both" then
		textFrame.isShown = true
		textFrame:Show()
		textFrame:SetOwner(UIParent, "ANCHOR_PRESERVE")
	end
	if VEM.Options.RangeFrameFrames == "radar" or VEM.Options.RangeFrameFrames == "both" then
