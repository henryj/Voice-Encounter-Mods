...
--				end
				if type(v.id) == "number" then -- creature ID
					local health, id = VEM:GetBossHP(v.id)
					if health then
						updateBar(v, health, GetRaidTargetIndex(id))
					end
				elseif type(v.id) == "string" then -- GUID
					local health, id = VEM:GetBossHPByGUID(v.id)
					if health then
						updateBar(v, health, GetRaidTargetIndex(id))
					end
...
function bossHealth:Show(name)
	if VEM.Options.DontShowHealthFrame then return end
