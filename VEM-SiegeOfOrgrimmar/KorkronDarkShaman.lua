...
function mod:FoulStreamTarget(targetname, uId)
	...
	if self:IsTanking(uId) then--Never target tanks, so if target is tank, that means scanning failed.
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxsl.ogg")
	else
		...
		if targetname == UnitName("player") then
			...
			specWarnFoulStream:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxsl.ogg") --小心水流
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 144005 and mod.Options.SoundEle then
		...
		specWarnToxicStormFix:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\wwsoon.ogg") --準備旋風
...
--		countdownFoulGeyser:Start()
	elseif args.spellId == 144070 and mod.Options.SoundEnh then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yszq.ogg") --元素之牆
	elseif args.spellId == 143973 then
		...
		sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_wmys.ogg") -- 5s後隕石爆炸
		if MyJS() then
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.ogg") --注意減傷
		else
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
		end
		sndWOP:Schedule(13, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
		sndWOP:Schedule(14, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
		sndWOP:Schedule(15, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 144288 and self:AntiSpam() then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dwtt.ogg") --毒霧圖騰
	elseif args.spellId == 144289 and self:AntiSpam() then
		warnFoulstreamTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_wltt.ogg") --污流圖騰
		if self.Options.RangeFrame then
			VEM.RangeCheck:Show(4)
		end
	elseif args.spellId == 144290 and self:AntiSpam() then
		warnAshflareTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hhtt.ogg") --灰火圖騰
	elseif args.spellId == 144291 and self:AntiSpam() then
		warnRustedIronTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xttt.ogg") --鏽鐵圖騰
	elseif args.spellId == 143990 and mod.Options.SoundEle then
		warnFoulGeyser:Show(args.destName)
		if args:IsPlayer() then
			yellFoulGeyser:Yell()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_rnkd.ogg") --软泥快打
			sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zbrn.ogg") --準備軟泥
		else
			if not mod:IsMeleeDps() then
				specWarnFoulGeyser:Show()
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_rnkd.ogg")
				sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zbrn.ogg")
			end
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 144089 then
		...
		if self.Options.SetIconOnToxicMists and args:IsDestTypePlayer() then--Filter further on icons because we don't want to set icons on grounding totems
			table.insert(toxicMistsTargetsIcons, VEM:GetRaidUnitId(VEM:GetFullPlayerNameByGUID(args.destGUID)))
	...
	elseif args.spellId == 144330 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(55, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.ogg") --快開自保
		end
		if self:AntiSpam(5, 1) then
			sndTL:Schedule(50, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tentl.ogg") --10秒後鐵牢
		end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 144330 and args:IsPlayer() then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.ogg")
	end
end
