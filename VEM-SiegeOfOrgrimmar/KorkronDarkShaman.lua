...
function mod:FoulStreamTarget(targetname, uId)
	...
	if self:IsTanking(uId) then--Never target tanks, so if target is tank, that means scanning failed.
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxsl.mp3")
	else
		...
		if targetname == UnitName("player") then
			...
			specWarnFoulStream:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxsl.mp3") --小心水流
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 144005 and mod.Options.SoundEle then
		...
		specWarnToxicStormFix:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\wwsoon.mp3") --準備旋風
...
--		countdownFoulGeyser:Start()
	elseif args.spellId == 144070 and mod.Options.SoundEnh then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yszq.mp3") --元素之牆
	elseif args.spellId == 143973 then
		...
		sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_wmys.mp3") -- 5s後隕石爆炸
		if MyJS() then
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
		else
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		end
		sndWOP:Schedule(13, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(14, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(15, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 144288 and self:AntiSpam() then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dwtt.mp3") --毒霧圖騰
	elseif args.spellId == 144289 and self:AntiSpam() then
		warnFoulstreamTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_wltt.mp3") --污流圖騰
		if self.Options.RangeFrame then
			VEM.RangeCheck:Show(4)
		end
	elseif args.spellId == 144290 and self:AntiSpam() then
		warnAshflareTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hhtt.mp3") --灰火圖騰
	elseif args.spellId == 144291 and self:AntiSpam() then
		warnRustedIronTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xttt.mp3") --鏽鐵圖騰
	elseif args.spellId == 143990 and mod.Options.SoundEle then
		warnFoulGeyser:Show(args.destName)
		if args:IsPlayer() then
			yellFoulGeyser:Yell()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_rnkd.mp3") --软泥快打
			sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zbrn.mp3") --準備軟泥
		else
			if not mod:IsMeleeDps() then
				specWarnFoulGeyser:Show()
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_rnkd.mp3")
				sndWOP:Schedule(28, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zbrn.mp3")
			end
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 144330 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(55, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.mp3") --快開自保
		end
		if self:AntiSpam(5, 1) then
			sndTL:Schedule(50, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tentl.mp3") --10秒後鐵牢
		end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 144330 and args:IsPlayer() then
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\holdit.mp3")
	end
end
