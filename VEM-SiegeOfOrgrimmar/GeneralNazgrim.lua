...
local function scanForMobs()
	if VEM:GetRaidRank() > 0 then--Cannot impliment counting because it seems there is too much variation between difficulties and it would be ugly
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(40, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\mobsoon.mp3") --準備小怪
	sndWOP:Schedule(41, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
--	countdownAdds:Start()
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 143872 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xff.mp3") --旋風斧
	...
	elseif args.spellId == 143431 then
		...
		if source == UnitName("target") or source == UnitName("focus") then
			...
			sndIFS:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\kickcast.mp3") --快打斷
		end
	...
	elseif args.spellId == 143473 then
		...
		if source == UnitName("target") or source == UnitName("focus") then
			...
			sndISM:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\kickcast.mp3") --快打斷
	...
	elseif args.spellId == 143502 then
		...
		if UnitExists("boss1") and UnitGUID("boss1") == args.sourceGUID and UnitDetailedThreatSituation("player", "boss1") then--threat check instead of target because we may be helping dps adds
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\execute.mp3") --斬殺
		end
		local executecd = 30
		...
		self:Schedule(executecd, function()
			if UnitExists("boss1") and UnitDetailedThreatSituation("player", "boss1") then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\executeready.mp3") --準備斬殺
			end
		end)
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 143589 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zdzt.mp3") --戰鬥姿態
	...
	elseif args.spellId == 143594 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_kbzt.mp3") --狂暴姿態
		if mod.Options.InfoFrame then
		...
		sndWOP:Schedule(55, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fyzb.mp3") --防禦姿態準備
	...
	elseif args.spellId == 143593 then
		...
		if (source == UnitName("target")) and (not UnitDebuff("player", sunder)) then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stopattack.mp3") --注意停手			
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fyzt.mp3") --防禦姿態
		end
		if mod.Options.InfoFrame then
			VEM.InfoFrame:SetHeader(GetSpellInfo(143593))
			VEM.InfoFrame:Show(4, "nazgrimpower")
		end
	...
	elseif args.spellId == 143536 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zqkd.mp3") --戰旗快打
	elseif args.spellId == 143474 then
		warnHealingTideTotem:Show()
		specWarnHealingTideTotem:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ttkd.mp3") --圖騰快打
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 143494 then
		...
		if args:IsPlayer() then
			if amount >= 3 then--At this point the other tank SHOULD be clear.
				...
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\sunderhigh.mp3") --破甲過高
			end
	...
	elseif args.spellId == 143484 then
		...
		sndWOP:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\specialsoon.mp3") --準備特別技能
...
--		countdownCoolingOff:Start()
	elseif args.spellId == 143480 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ckkp.mp3") --快跑 刺客點你
		else
	...
	elseif args.spellId == 143882 then
		if args:IsPlayer() then
			...
			sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3")
		end
	elseif args.spellId == 143475 and not args:IsDestTypePlayer() then
		...
		sndDS:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dun.mp3") --驅散大地盾
...
--		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killmob.mp3") --小怪快打
		sndWOP:Schedule(39, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\mobsoon.mp3")
...
--		countdownAdds:Start()
	...
	elseif msg == L.allForces then
		...
		specWarnAdds:Show(0)
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\mobsoon.mp3")
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3") --第二阶段
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 143500 then--Faster than combat log by 0.3-0.5 seconds
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yzkd.mp3") --餘震快躲
	end
end
