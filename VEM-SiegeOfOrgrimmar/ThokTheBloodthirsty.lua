...
function mod:OnCombatStart(delay)
	...
	if self:IsDifficulty("lfr25") then
		...
		sndPX:Schedule(18-delay, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
		sndPX:Schedule(19-delay, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
	else
		timerDeafeningScreechCD:Start(-delay, 1)
		sndPX:Schedule(10-delay, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.ogg")	
		sndPX:Schedule(11-delay, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
		sndPX:Schedule(12-delay, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 143343 then--Assumed, 2 second channel but "Instant" cast flagged, this generally means SPELL_AURA_APPLIED
		...
		sndPX:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.ogg")
		...
		if self:IsDifficulty("lfr25") then
			...
			sndPX:Schedule(17, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
			sndPX:Schedule(18, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		else
			...
			if screechTimers[screechCount] and screechTimers[screechCount] > 2.2 then
				if screechTimers[screechCount] > 3.2 then
					sndPX:Schedule(screechTimers[screechCount+1] - 3.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.ogg")
				end
				sndPX:Schedule(screechTimers[screechCount+1] - 2.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
				sndPX:Schedule(screechTimers[screechCount+1] - 1.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
				sndPX:Schedule(screechTimers[screechCount+1] - 0.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
			end
		end
	...
	elseif args.spellId == 31821 then --BH ADD
		...
		if mod:IsManaUser() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_qcgh.ogg") --虔誠光環
		end
	end
end
...
--DELETE		soundFixate:Play()
	...
	elseif args.spellId == 143800 and args:IsPlayer() then
		...
		if amount >= 3 then
			...
			if amount == 4 then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zybd.ogg") --注意冰凍
			end
		end
	elseif args.spellId == 143777 then
		...
		if self:AntiSpam(3, 1) then
			...
			if mod:IsDps() then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bmkd.ogg") -- 冰墓快打
			end
		end
	elseif args.spellId == 145974 then
		...
		if (source == UnitName("target") or source == UnitName("focus")) and mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\enrage.ogg") -- 激怒
		elseif mod:CanRemoveEnrage() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\trannow.ogg") -- 注意寧神
		end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 143440 then
		...
		if self:IsDifficulty("lfr25") then
			...
			sndPX:Schedule(18, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
			sndPX:Schedule(19, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
		else
			timerDeafeningScreechCD:Start(nil, 1)
			sndPX:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.ogg")
			sndPX:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
...
--DELETE	soundBloodFrenzy:Play()
	...
	elseif spellId == 143971 then
		...
		timerAcidBreathCD:Start()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dyqh.ogg") --毒液恐龍
		phase = 1
	elseif spellId == 143968 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bsqh.ogg") --冰霜恐龍
		phase = 1
	elseif spellId == 143970 then
		...
		timerScorchingBreathCD:Start()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lyqh.ogg") --烈焰恐龍
		phase = 1
	end
end
