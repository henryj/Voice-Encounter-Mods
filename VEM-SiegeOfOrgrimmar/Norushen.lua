...
local function fixdebuffremovebug(checkplayer)
    if UnitDebuff(checkplayer, GetSpellInfo(144849)) or UnitDebuff(checkplayer, GetSpellInfo(144850)) or UnitDebuff(checkplayer, GetSpellInfo(144851)) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\otherout.mp3")--隊友出场
	end
end
...
function mod:OnCombatStart(delay)
	...
	self:Schedule(21, function()
		if not playerInside then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_sxzb.mp3")--射線準備
	...
	if self.Options.InfoFrame2 then
		VEM.InfoFrame:SetHeader(GetSpellInfo(144452))
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 144482 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_slkd.mp3") --撕裂快躲
	...
	elseif args.spellId == 144628 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mjkd.mp3") --猛擊快躲
...
--		countdownLingeringCorruption:Start()
	...
	elseif args:IsSpellID(144849, 144850, 144851) then--Look Within
		...
		if args.destName == mod.Options.prevplayer then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mbrc.mp3")--隊友入场			
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 146179 then--Frayed
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3")--二階段準備
		sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmobsoon.mp3")--準備大怪
		if mod:IsDps() then
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killbigmob.mp3")--大怪快打
		else
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmob.mp3")--大怪出現
		end
	end
end
...
function mod:OnSync(msg)
	...
	elseif msg == "ManifestationDied" and not playerInside and self:AntiSpam(1) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmobsoon.mp3")--準備大怪
		if mod:IsDps() then
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killbigmob.mp3")--大怪快打
		else
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmob.mp3")--大怪出現
