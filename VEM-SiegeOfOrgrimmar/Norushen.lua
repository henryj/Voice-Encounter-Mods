...
local function fixdebuffremovebug(checkplayer)
    if UnitDebuff(checkplayer, GetSpellInfo(144849)) or UnitDebuff(checkplayer, GetSpellInfo(144850)) or UnitDebuff(checkplayer, GetSpellInfo(144851)) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\otherout.ogg")--隊友出场
	end
end
...
function mod:OnCombatStart(delay)
	...
	self:Schedule(21, function()
		if not playerInside then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_sxzb.ogg")--射線準備
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 144482 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_slkd.ogg") --撕裂快躲
	...
	elseif args.spellId == 144628 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mjkd.ogg") --猛擊快躲
...
--		countdownLingeringCorruption:Start()
	...
	elseif args:IsSpellID(144849, 144850, 144851) then--Look Within
		...
		if args.destName == mod.Options.prevplayer then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mbrc.ogg")--隊友入场			
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 146179 then--Frayed
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ptwo.ogg")--二階段準備
		sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmobsoon.ogg")--準備大怪
		if mod:IsDps() then
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killbigmob.ogg")--大怪快打
		else
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmob.ogg")--大怪出現
		end
	end
end
...
function mod:OnSync(msg)
	...
	elseif msg == "ManifestationDied" and not playerInside and self:AntiSpam(1) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmobsoon.ogg")--準備大怪
		if mod:IsDps() then
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killbigmob.ogg")--大怪快打
		else
			sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bigmob.ogg")--大怪出現
