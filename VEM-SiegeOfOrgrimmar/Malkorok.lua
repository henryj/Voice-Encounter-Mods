local mod	= VEM:NewMod(846, "VEM-SiegeOfOrgrimmar", nil, 369)
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(67, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yszb.mp3") --亞煞極之息準備
...
--			soundDisplacedEnergy:Play()
			...
			sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
			sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
			sndWOP:Schedule(4.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(5.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(8.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
		if self.Options.HudMAP then
			DisplacedMaker[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 8, 9, 0, 1, 0, 0.6):Appear():RegisterForAlerts():Rotate(360, 9.5))
		end
		if self.Options.SetIconOnDisplacedEnergy and args:IsDestTypePlayer() then--Filter further on icons because we don't want to set icons on grounding totems
			table.insert(displacedEnergyTargetsIcons, VEM:GetRaidUnitId(VEM:GetFullPlayerNameByGUID(args.destGUID)))
...
function mod:SPELL_AURA_REMOVED(args)
	if args.spellId == 142913 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3")
			...
			if self.Options.RangeFrame and playerDebuffs >= 1 then
				VEM.RangeCheck:Show(10, debuffFilter)--Change to debuff filter based check since theirs is gone but there are still others in raid.
			end
		end
...
--		countdownImplodingEnergy:Start()
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yhz.mp3") --圓弧斬快躲
		sndWOP:Schedule(5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fscq.mp3") --分散踩圈
		if MyJS() then
			sndWOP:Schedule(5.8, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
			sndWOP:Schedule(6.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
		end
