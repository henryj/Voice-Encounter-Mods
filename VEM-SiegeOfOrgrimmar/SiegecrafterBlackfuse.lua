...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 144208 then
		...
		sndWOP:Schedule(3, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_so_fmjd.mp3") --伐木機快打
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 143639 then--Missile Activation
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\watchstep.mp3") --鑽頭準備
	end
end
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, npc, _, _, target)
	if msg == L.newWeapons or msg:find(L.newWeapons) then
		...
		if MyCS() then
			sndWOP:Schedule(32, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_so_kqcs.mp3") --快去傳送帶
		else			
			VEM:Schedule(32, function() VEM:PlayCountSound(weapon + 1) end)			
		end
...
--		timerAutomatedShredderCD:Start()
	elseif msg:find("spell:145774") then
		print("VEM Debug: Drycoded Fallback, in case Overcharge is hidden from combat log", npc or "noNPC", target or "noTarget")
	end
end
