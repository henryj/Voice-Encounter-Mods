local mod	= VEM:NewMod(865, "VEM-SiegeOfOrgrimmar", nil, 369)
...
local function showspellinfo(weaponnum)
	if mod.Options.InfoFrame then		
		...
		elseif weaponnum == 6 then
			VEM.InfoFrame:Show(1, "other", dl.." / "..jg, dl)
		elseif weaponnum == 7 then
			VEM.InfoFrame:Show(1, "other", jg.." / "..fd, dl)
		elseif weaponnum == 8 then
			VEM.InfoFrame:Show(1, "other", dl.." / "..dc, fd)
		elseif weaponnum == 9 then
			VEM.InfoFrame:Show(1, "other", jg.." / "..dl, jg)
		elseif weaponnum == 10 then
			VEM.InfoFrame:Show(1, "other", dl.." / "..jg, fd)
		elseif weaponnum == 11 then
			VEM.InfoFrame:Show(1, "other", fd.." / "..dc, fd)
...
function mod:DeathFromAboveTarget(sGUID)
	...
	if targetname == UnitName("player") then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_sctj.mp3") --死從
	else
		...
		if uId then
			...
			if inRange and inRange < 20 then
				...
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_sctj.mp3")
			end
		end
	end
end
...
local function findLaser()
	for uId in VEM:GetGroupMembers() do
		...
		if UnitDebuff(uId, spellName) then
			print("VEM DEBUG: Possible match to laser targeting using "..spellName)
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 144208 then
		...
		sndFMD:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fmjd.mp3") --伐木機快打
	end
end
...
--		timerShockwaveMissileActive:Start()
	...
	elseif args.spellId == 145774 then
		...
		if cid == 71638 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fdqh.mp3") --飛彈強化
		elseif cid == 71752 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_jgqh.mp3") --激光強化
		elseif cid == 71696 then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ccqh.mp3") --磁場強化
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dlqh.mp3") --地雷強化
		end
	elseif args.spellId == 145444 then
		...
		if OverloadCount == 3 then
			sndCZ4:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_4cz.mp3") --超載4
			sndCZ4:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
			sndCZ4:Schedule(8.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
		elseif OverloadCount == 4 then
			sndCZ5:Schedule(6, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_5cz.mp3") --超載5
			sndCZ5:Schedule(7.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
			sndCZ5:Schedule(8.2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
		else
			sndCZ:Schedule(7, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --準備AOE
		end
	end
end
...
function mod:SPELL_SUMMON(args)
	if args.spellId == 143641 then--Missile Launching
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fdzb.mp3") --飛彈準備
	end
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 145365 then
		...
		if mod:IsTank() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\speedup.mp3") --首領加速
		end
	...
	elseif args.spellId == 145269 then
		if self:AntiSpam(30, 3) then --BHFIX
			...
			sndDL:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\killmine.mp3") --地雷快打
			DLneedshow = true
		end
...
--[[	elseif args.spellId == 143867 then
	...
	elseif args.spellId == 144466 and self:AntiSpam(15, 1) then--Only way i see to detect magnet activation, antispam is so it doesn't break if a player dies during it.
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dczd.mp3") --電磁震蕩
...
function mod:UNIT_DIED(args)
	...
	if cid == 71591 then
		...
		sndCZ:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3")
		sndCZ4:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_4cz.mp3")
		sndCZ4:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
		sndCZ5:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_5cz.mp3")
		sndCZ5:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
	end
end
...
function mod:RAID_BOSS_WHISPER(msg)
	if msg:find("spell:143266") then--Target scanning works on this one, but is about 1 second slower than emote. emote is .2 seconds after SPELL_CAST_START, but target scanning can't grab right target until like 1.0 or 1.2 sec into cast
		...
		if mod.Options.LTFD then
			VEM:ShowLTSpecialWarning(143266, 1, 0, 0, 1, 143266, 2)
		end
...
--			yellCrawlerMineFixate:Yell()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dlsd.mp3") --地雷鎖定
		end
	elseif msg:find("Ability_Siege_Engineer_Superheated") then
		if self:AntiSpam(3, 10) then
			...
			VEM.Flash:Shake(1, 0, 0)
			yellLaserFixate:Yell()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\laserrun.mp3") --快跑,激光點你
			if mod.Options.LTFD then
				VEM:ShowLTSpecialWarning(143828, 1, 0, 0, 1, 143828, 2)
			end
...
local function showheroichx()
	if myweapon then
		local cdtext = ""
		VEM:ShowLTSpecialWarning(cdtext, 0, 1, 0, nil, 143856, nil, 3)
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, npc, _, _, target)
	if msg == L.newWeapons or msg:find(L.newWeapons) then
		...
		if weapon == 1 and (not mod.Options.SoundCS2) then
			sndWOP:Schedule(30, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tenwq.mp3") --10秒後武器啟動
		end
		
		if weapon % 2 == 1 then
			sndCS2:Schedule(30, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_2zu.mp3") --2組
			sndCS2:Schedule(31, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_2zu.mp3")
		end
		
		if weapon % 2 == 0 then
			sndCS1:Schedule(30, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_1zu.mp3") --1組
			sndCS1:Schedule(31, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bh_1zu.mp3")
		end
		
		if MyCS() or (mod.Options.optCS == "CSALL") then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_kqcs.mp3") --快去傳送帶
			...
			if mod.Options[weaponoption] == "killdl" then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dddl.mp3") --打掉地雷
				sndWOP:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dddl.mp3")
			...
			elseif mod.Options[weaponoption] == "killfd" then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ddfd.mp3") --打掉飛彈
				sndWOP:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ddfd.mp3")
				if mod.Options.ShowDps then
					self:Schedule(40, function()
						table.wipe(Damagesort)
			...
			elseif mod.Options[weaponoption] == "killjg" then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ddjg.mp3") --打掉激光
				sndWOP:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ddjg.mp3")
				if mod.Options.ShowDps then
					self:Schedule(40, function()
						table.wipe(Damagesort)
			...
			elseif mod.Options[weaponoption] == "killdc" then
				sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dddc.mp3") --打掉電磁鐵
				sndWOP:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dddc.mp3")
				if mod.Options.ShowDps then
					self:Schedule(40, function()
						table.wipe(Damagesort)
	...
	elseif msg == L.newShredder or msg:find(L.newShredder) then
		...
		if mod:IsTank() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fmjc.mp3") --伐木機出現
		end
	end
end
	...
	if msg == "LaserTarget" and guid then		
		...
		if guid ~= UnitGUID("player") then
			...
			if self.Options.HudMAP then
				...
				lasermaker[targetName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("targeting", targetName, 5, 5, 1, 0 ,0 ,1):SetLabel(spelltext))
			end
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxhx.mp3") --小心火線
		end
	end
end
