...
local function MalkorokPShow()
	...
	if malkorok_raid_marks[1]==nil then
		for i=1,40 do
			...
			malkorok_raid_marks[i]:SetBackdrop({bgFile = "Interface\\AddOns\\VEM-SiegeOfOrgrimmar\\Textures\\blip.tga",tile = true,tileSize = 16});
...
local function MalkorokLoad()
	...
	malkorok_mainframe:SetBackdrop({bgFile = "Interface/Tooltips/UI-Tooltip-Background",edgeFile = "Interface\\AddOns\\VEM-SiegeOfOrgrimmar\\Textures\\border.tga",tile = false,edgeSize = 8});
	malkorok_mainframe:SetBackdropColor(0,0,0,0.5);
	malkorok_mainframe:SetBackdropBorderColor(0.2,0.2,0.2,0.4);
	...
	for i=1,6 do 
		...
		malkorok_mainframe_pie[i].tex:SetTexture("Interface\\AddOns\\VEM-SiegeOfOrgrimmar\\Textures\\Pie"..tostring(i)..".tga", true)
	...
	malkorok_mainframe:SetScript("OnMouseDown", function(self,button)
		...
		if j1o < 0 and j2o < 0 then
			if malkorok_mainframe_lock == nil then
				...
				malkorok_mainframe.Lock.Texture:SetTexture("Interface\\AddOns\\VEM-SiegeOfOrgrimmar\\Textures\\lock.tga", false)
				malkorok_mainframe:SetMovable(false)
				ExRT["Bossmods"]["MalkorokLock"] = true
			else
				malkorok_mainframe_lock = nil
				malkorok_mainframe.Lock.Texture:SetTexture("Interface\\AddOns\\VEM-SiegeOfOrgrimmar\\Textures\\un_lock.tga", false)
				malkorok_mainframe:SetMovable(true)
		...
		if (numpie > 0) and VEM.MalHelperEnabled then
