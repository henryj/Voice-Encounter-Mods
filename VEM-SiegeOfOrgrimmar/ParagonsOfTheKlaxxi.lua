...
local function warnMutatedTargets()	
	...
	if mod:AntiSpam(5, 1) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tb.mp3") --突變
		if mutatecount == 1 then
			sndWOP:Schedule(0.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		elseif mutatecount == 2 then
			sndWOP:Schedule(0.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		...
		elseif mutatecount == 4 then
			sndWOP:Schedule(0.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		elseif mutatecount == 5 then
			sndWOP:Schedule(0.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
		elseif mutatecount == 6 then
			sndWOP:Schedule(0.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
		end
	...
	for i = 1, 5 do
		...
		if UnitExists(unitID) and mod:GetCIDFromGUID(UnitGUID(unitID)) == 71161 then
			if (not UnitExists(unitID.."target")) or not mod:IsTanking(unitID.."target", unitID) then
				...
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_sctj.mp3") --死從天降
		...
		if UnitExists(unitID) and not activeBossGUIDS[UnitGUID(unitID)] then--Check if new units exist we haven't detected and added yet.
			...
			if cid == 71161 then--Kil'ruk the Wind-Reaver
				...
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lfz.mp3") --掠風者參戰
				end
			elseif cid == 71157 then--Xaril the Poisoned-Mind
				if UnitDebuff("player", GetSpellInfo(142931)) then vulnerable = true end
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dxz.mp3") --毒心者參戰
				end
			elseif cid == 71156 then--Kaz'tik the Manipulator
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_czz.mp3") --操縱者參戰
					sndWOP:Schedule(1, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ylcz.mp3")
				end
			elseif cid == 71155 then--Korven the Prime
				timerShieldBashCD:Start(19)--20seconds from REAL IEEU
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zzz.mp3") --至尊者參戰
				end
			elseif cid == 71160 then--Iyyokuk the Lucid
				timerInsaneCalculationCD:Start()
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mcz.mp3") --明澈者參戰
				end
			elseif cid == 71154 then--Ka'roz the Locust
				...
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bsh.mp3") --暴食蝗參戰
				end
			elseif cid == 71152 then--Skeer the Bloodseeker
				...
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_nxz.mp3") --覓血者參戰
				end
			elseif cid == 71158 then--Rik'kal the Dissector
				...
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_qgz.mp3") --切割者參戰					
				end
			elseif cid == 71153 then--Hisek the Swarmkeeper
				...
				if activetime >= 15 then
					sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_cqws.mp3") --蟲群衛士參戰
				end
			end
		end
	end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 142725 then
		...
		if UnitDebuff("player", GetSpellInfo(142532)) then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lsbz.mp3") --藍色爆炸準備
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lsch.mp3") --藍色催化
		end
	elseif args.spellId == 142726 then
		...
		if UnitDebuff("player", GetSpellInfo(142533)) then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hsch.mp3") --紅色催化
	...
	elseif args.spellId == 142727 then
		...
		if UnitDebuff("player", GetSpellInfo(142534)) then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hsdq.mp3") --黃色毒氣準備
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_ysch.mp3") --黃色催化
			if self.Options.HudMAP then
				for i = 1, VEM:GetNumGroupMembers() do
	...
	elseif args.spellId == 142728 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_csch.mp3") --橙色催化
		sndWOP:Schedule(3, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bzhh.mp3") --爆炸火環準備
	...
	elseif args.spellId == 142729 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zsch.mp3") --紫色催化
	elseif args.spellId == 142730 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lvsch.mp3") --綠色催化
		sndWOP:Schedule(2, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xxls.mp3") --小心綠水
	...
	elseif args.spellId == 142416 then
		...
		if caled then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\linesoon.mp3") --準備連線
			caled = false
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_yllx.mp3") --遠離連線
		end
...
--[[DELETE		if self.Options.RangeFrame then
	...
	elseif args.spellId == 143280 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zbrn.mp3")
		sndWOP:Schedule(3.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_rnkd.mp3")
	...
	elseif args.spellId == 143243 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_mop_ybzb.mp3") --音波準備
		if self.Options.LTIP and (not showtank) then
	...
	elseif args.spellId == 148676 then
		warnReave:Show()
		specWarnReave:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_xft.mp3") --旋風準備
		timerReaveCD:Start()
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 142532 and args:IsPlayer() then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lsds.mp3") --藍色毒素
		havecolor = true
	elseif args.spellId == 142533 and args:IsPlayer() then
		specWarnToxicRed:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hsds.mp3") --紅色毒素
		havecolor = true
	elseif args.spellId == 142534 and args:IsPlayer() then
		specWarnToxicYellow:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_huds.mp3") --黃色毒素
		havecolor = true
--[[	elseif args.spellId == 142547 and args:IsPlayer() then
	...
	elseif args.spellId == 142671 then
		...
		if args.IsPlayer() then
			...
			yellMesmerize:Yell()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_nbcm.mp3") --你被催眠
		else
			specWarnKunchongs:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_czkd.mp3") --蟲子快打
	...
	elseif args.spellId == 142564 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hupd.mp3") --琥珀快打
...
--			countdownEncaseInAmber:Start()
	...
	elseif args.spellId == 143939 then
		...
		if args.IsPlayer() then
			specWarnGouge:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zj.mp3") --鑿擊
		else
			specWarnGougeOther:Show(args.destName)
			if mod:IsHealer() then
				sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\tankheal.mp3") --注意刷坦
			end
		end
	elseif args.spellId == 143974 then
		...
		if args.IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_dj.mp3") --盾擊
		else
	...
	elseif args.spellId == 143759 then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tzhp.mp3") --投擲琥珀
		if self.Options.LTIP and (not showtank) then
	...
	elseif args.spellId == 143337 then
		...
		if args.IsPlayer() then
			specWarnMutate:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_bsjx.mp3") --變身巨蠍
			timerMutate:Start()
		end
	...
	elseif args.spellId == 142948 then
		...
		if args.IsPlayer() then
			specWarnAim:Show()
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_nbmz.mp3") --你被瞄準
			yellAim:Yell()
		else
			specWarnAimOther:Show(args.destName)
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_fdyb.mp3") --準備分擔音波
...
--		warnCalculated:Show(targetname)
		...
		if target == UnitName("player") then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lxdn.mp3") --連線點你
			yellCalculated:Yell()
			caled = true
		end
		if self:IsDifficulty("heroic10", "heroic25") then
			...
			if shape then
				if shape == resultshape or color == resultcolor or number == resultnumber then
					if target ~= UnitName("player") then
						specWarnCalculated:Show()
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_lxdn.mp3")
						yellCalculated:Yell()
						caled = true
					end
				else
			...
			--[[if #ResultMeleeTargets < 7 then
				...
				for i = 1, needranged do
					if ResultRangedDPSTargets[i] == UnitName("player") then
						sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\gather.mp3")
					end
				end
			end]]
		end		
