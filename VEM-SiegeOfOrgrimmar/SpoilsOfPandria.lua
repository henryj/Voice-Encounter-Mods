...
local function warnspecmob(guid)
	...
	if cid == 71382 then
		if mod:IsDps() or mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hpkd.ogg") --花瓶快打
		end
	elseif cid == 71395 then
		if mod:IsDps() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mxkd.ogg") --魔像快打
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mxcx.ogg") --魔像出现
		end
	elseif cid == 71385 then
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tdsd.ogg") --投彈手快打
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 145288 and checkTankDistance(args.sourceGUID) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stepring.ogg") --注意踩圈
	elseif args.spellId == 145461 and checkTankDistance(args.sourceGUID) then
		warnEnergize:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_llfw.ogg") --力量符文
	elseif args.spellId == 142934 and checkTankDistance(args.sourceGUID) then
		warnTorment:Show()
		specWarnTorment:Show()
		if mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zmcx.ogg") --折磨出現
		end
	elseif args.spellId == 142539 and checkTankDistance(args.sourceGUID) then
		...
		if mod:IsDps() or mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zhcq.ogg") --召喚蟲群
		end
...
--			countdownSetToBlow:Start()
			...
			sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bombnow.ogg") --準備爆炸
			sndWOP:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.ogg")
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.ogg")
			sndWOP:Schedule(13, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.ogg")
			sndWOP:Schedule(14, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.ogg")
			if self.Options.RangeFrame then
...
--		countdownSetToBlow:Cancel()
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bombnow.ogg")
