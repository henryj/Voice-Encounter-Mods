local mod	= VEM:NewMod(870, "VEM-SiegeOfOrgrimmar", nil, 369)
...
--Berserk Timer stuff
local berserkTimer				= mod:NewTimer(480, VEM_CORE_GENERIC_TIMER_BERSERK, 28131, nil, "timer_berserk")
local countdownBerserk			= mod:NewCountdown(20, 26662, nil, nil, 10, nil, true)
local berserkWarning1			= mod:NewAnnounce(VEM_CORE_GENERIC_WARNING_BERSERK, 1, nil, "warning_berserk", false)
local berserkWarning2			= mod:NewAnnounce(VEM_CORE_GENERIC_WARNING_BERSERK, 4, nil, "warning_berserk", false)
...
local function warnspecmob(guid)
	...
	if cid == 71382 then
		if mod:IsDps() or mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_hpkd.mp3") --花瓶快打
		end
	elseif cid == 71395 then
		if mod:IsDps() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mxkd.mp3") --魔像快打
		else
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_mxcx.mp3") --魔像出现
		end
	elseif cid == 71385 then
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_tdsd.mp3") --投彈手快打
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 145288 and checkTankDistance(args.sourceGUID) then
		...
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\stepring.mp3") --注意踩圈
	elseif args.spellId == 145461 and checkTankDistance(args.sourceGUID) then
		warnEnergize:Show()
		sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_llfw.mp3") --力量符文
	elseif args.spellId == 142934 and checkTankDistance(args.sourceGUID) then
		warnTorment:Show()
		specWarnTorment:Show()
		if mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zmcx.mp3") --折磨出現
		end
	elseif args.spellId == 142539 and checkTankDistance(args.sourceGUID) then
		...
		if mod:IsDps() or mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\ex_so_zhcq.mp3") --召喚蟲群
		end
...
--			countdownSetToBlow:Start()
			...
			sndWOP:Schedule(10, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bombnow.mp3") --準備爆炸
			sndWOP:Schedule(11, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(12, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(13, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(14, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			if self.Options.RangeFrame then
				VEM.RangeCheck:Show(10)--Range assumed, spell tooltips not informative enough
				self:Schedule(16, hideRangeFrame)
			end
			if self.Options.LTZD then
				VEM:ShowLTSpecialWarning(145987, 1, 0, 0, 1, 145987, 15, 15)
			end
		end
...
--		countdownSetToBlow:Cancel()
		...
		sndWOP:Cancel("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\bombnow.mp3")
...
function mod:SPELL_DAMAGE(sourceGUID, sourceName, _, _, destGUID, destName, _, _, spellId)
	...
	if self.Options.Filterarea then
		if (not bossDamageTarget[sourceGUID]) then
			local uId = VEM:GetRaidUnitId(destName)
			if uId then
		...
		if (not bossDamageTarget[destGUID]) then
			local uId = VEM:GetRaidUnitId(sourceName)
			if uId then
