...
function mod:SPELL_CAST_START(args)
	if args.spellId == 143436 then
		...
		specWarnBreath:Show()
		if mod:IsTank() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_so_zbcj.mp3") --準備衝擊
		end
		timerBreathCD:Start()
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args:IsSpellID(143579) and args:IsPlayer() then
		if needwarnsafe then
			sndPZ:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\safenow.mp3") --安全
			needwarnsafe = false
		end
	end
end
	...
	if spellId == 145377 or spellId == 143498 then
		if needwarndr then
			...
			if MyJS() then
				...
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\holdit.mp3")
			end
		end
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	...
	elseif spellId == 143578 then--Swelling Corruption
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_so_pz.mp3") --膨脹
	end
end
