local mod	= VEM:NewMod(861, "VEM-Pandaria", nil, 322, 1)
...
--		timerBurningSoulCD:Start()
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3") --离开人群
			sndWOP:Schedule(1.5, "Interface\\AddOns\\"..VEM.Options.CountdownVoice.."\\runout.mp3")
		end
		if self.Options.HudMAP then
			SoulMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 10, 10, 0, 1, 0, 0.4):Appear():RegisterForAlerts():Rotate(360, 10))
		end
		--ADD HUD END
